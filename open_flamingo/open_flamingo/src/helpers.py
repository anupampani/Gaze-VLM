...
class GazeOverlayGenerator(nn.Module):
...
class GazeRegressor(nn.Module):
    def __init__(self, dim, num_latents=1, depth=2, dim_head=64, heads=8):
        super().__init__()

        self.latents = nn.Parameter(torch.randn(num_latents, dim))  # Learnable gaze query
        ...
        self.decoder = nn.TransformerDecoder(self.layers[0], num_layers=depth)
        self.norm = nn.LayerNorm(dim)

    def forward(self, x):
        """
        Args:
            x: ViT embeddings (b, T, v, d) -> (batch, time, num_patches, dim)
        Returns:
            Gaze embedding (b, T, 1, d)
        """
        b, T, v, d = x.shape

        # Flatten across batch and time for transformer decoder
        x = rearrange(x, "b T v d -> (b T) v d")

        # Expand learnable latent (gaze query) across batch and time
        latents = repeat(self.latents, "n d -> (b T) n d", b=b, T=T)

        # Apply transformer decoder
        gaze_embedding = self.decoder(latents, x)  # (b*T, n, d)

        # Normalize and reshape
        gaze_embedding = self.norm(gaze_embedding)
        gaze_embedding = rearrange(gaze_embedding, "(b T) n d -> b T n d", b=b, T=T)

        return gaze_embedding
...
#         # New: linear projection to form global query from vision features
...
#         Returns:
#             attn_weights, output: (b, T, 1, d)
#         """
#         b, T, F, v, d = x.shape
#         x = rearrange(x, "b T F v d -> b T (F v) d")  # (b, T, N, d)

#         # Step 1: Aggregate across time and space to get a global query vector
#         x_flat = x.view(b, T * F * v, d)              # (b, T*N, d)
#         global_q = x_flat.mean(dim=1)                 # (b, d)
#         global_q = self.global_query_proj(global_q)   # (b, d)

#         # Step 2: Repeat global query across time
#         q = global_q.unsqueeze(1).unsqueeze(2).repeat(1, T, 1, 1)  # (b, T, 1, d)

#         for attn, ff in self.layers:
#             attn_weights, attn_score = attn(x, q)  # (b, T, 1, d)
#             q = attn_score + q
#             q = ff(q) + q

#         return attn_weights, self.norm(q)
...
class GazeAttention(nn.Module):
    ...
    def forward(self, vision_x, gaze_x):
        """
        Args:
            vision_x: visual features of shape (B, T, N, D)
            gaze_x: gaze features for query (B, n_queries, D)
        Returns:
            attn: attention map (B, heads, n_queries, T*N)
            out: attention output (B, n_queries, D)
        """
        B, T, N, D = vision_x.shape  # e.g., B x T x N x D
        H = self.heads

        # Flatten vision tokens across time and patches: (B, T*N, D)
        vision_x = vision_x.view(B, T * N, D)

        vision_x = self.norm_media(vision_x)
        gaze_x = self.norm_gaze(gaze_x)

        # Project queries, keys, values
        q = self.to_q(gaze_x)               # (B, n_queries, H*dim_head)
        kv = self.to_kv(vision_x)           # (B, T*N, 2*H*dim_head)
        k, v = kv.chunk(2, dim=-1)

        # Reshape for multi-head attention
        q = q.view(B, -1, H, -1).transpose(1, 2)  # (B, H, n_queries, dim_head)
        k = k.view(B, -1, H, -1).transpose(1, 2)  # (B, H, T*N, dim_head)
        v = v.view(B, -1, H, -1).transpose(1, 2)  # (B, H, T*N, dim_head)

        q = q * self.scale

        # Attention: (B, H, n_queries, T*N)
        sim = einsum("bhid,bhjd->bhij", q, k)
        sim = sim - sim.amax(dim=-1, keepdim=True).detach()
        attn = sim.softmax(dim=-1)

        # Weighted sum
        out = einsum("bhij,bhjd->bhid", attn, v)  # (B, H, n_queries, dim_head)
        out = out.transpose(1, 2).contiguous().view(B, -1, H * (D // H))  # (B, n_queries, D)
