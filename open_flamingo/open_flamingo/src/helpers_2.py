...
class AttentionSampler(nn.Module):
        ...
        *,
        ...
        for _ in range(depth):
            self.layers.append(
                nn.ModuleList(
                    [
                        NewAttention(dim=dim, dim_head=dim_head, heads=heads),
                        FeedForward(dim=dim, mult=ff_mult),
                    ]
                )
            )
    ...
    def forward(self, x):
        ...
        for attn, ff in self.layers:
            attn_weights,attn_score= attn(x)
            x = attn_score + x
            x = ff(x) + x
        return attn_weights,self.norm(x)
...
class NewAttention(nn.Module):
    def __init__(self, *, dim, dim_head=64, heads=8,patches=256,n_queries=1):
        ...
        self.to_q = nn.Linear(dim, inner_dim, bias=False)  # Vision features as queries
