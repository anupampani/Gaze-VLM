...
import torch
import wandb
from data import get_dataset_gaze
...
def fsdp_main(rank, world_size, args):
    ...
    valid_dataset_info = get_dataset_gaze(tokenizer, clip_processor, 'validation', rank, world_size)
    ...
    checkpoint_path = os.path.join("insert path/open_flamingo/open_flamingo/models/gaze_model/gaze_model_checkpoint_10_token.pth")
    if os.path.isfile(checkpoint_path):
        dist.barrier()
        resume_checkpoint=1
        start_epoch ,checkpoint = load_checkpoint_gaze("insert path/open_flamingo/open_flamingo/models/gaze_model/gaze_model_checkpoint_10_token.pth")
        if rank==0:
            model.load_state_dict(checkpoint['model_state_dict'],strict=False)
    ...
    #training 
        ...
        trainloss= train_one_epoch_gaze(args, model, epoch, train_dataset, tokenizer,optimizer, scheduler, rank,wandb)  # Assuming wandb or logging handled within
        train_loss.append(trainloss)

        validloss= validate_gaze(args, model, valid_dataset, tokenizer, rank)

        print("saving started")
        if( validloss < best_loss):
            best_loss=validloss
            save_checkpoint_gaze(model, optimizer, scheduler, epoch, "insert path/open_flamingo/open_flamingo/models/gaze_model/gaze_model_checkpoint_11_token.pth")
        if rank==0:
            save_loss_to_file(trainloss,f"{args.save_dir}/gaze_train_loss.txt")
            save_loss_to_file(validloss,f"{args.save_dir}/gaze_valid_loss.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir",type=str,default="insert path/open_flamingo/open_flamingo/models/gaze_model")
