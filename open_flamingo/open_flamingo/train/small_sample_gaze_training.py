...
import torch
import wandb
from data import get_dataset_gaze,get_small_dataset,get_small_dataset_gaze
...
def fsdp_main(rank, world_size, args):
    ...
    #dataset loading 
    train_dataset_info = get_small_dataset_gaze(tokenizer, clip_processor, 'train', rank, world_size,sample_limit=32*10)
    ...
    checkpoint_path = os.path.join("insert path/open_flamingo/open_flamingo/models/small_training_gaze/0_001_heatmapfirst_last_checkpoint.pth")
    if os.path.isfile(checkpoint_path):
        dist.barrier()
        resume_checkpoint=1
        start_epoch ,checkpoint = load_checkpoint_gaze("insert path/open_flamingo/open_flamingo/models/small_training_gaze/0_001_heatmapfirst_last_checkpoint.pth")
        if rank==0:
            model.load_state_dict(checkpoint['model_state_dict'],strict=False)
    ...
    #training 
        ...
        if(trainloss < best_loss):
            best_loss=trainloss
            save_checkpoint_gaze(model, optimizer, scheduler, epoch, "insert path/open_flamingo/open_flamingo/models/small_training_gaze/0_001_heatmapfirst2_last_checkpoint.pth")
        if rank==0:
            save_loss_to_file(trainloss,f"{args.save_dir}/small_gaze_train_loss.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir",type=str,default="insert path/open_flamingo/open_flamingo/models/small_training_gaze")
