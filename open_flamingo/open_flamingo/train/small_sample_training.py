...
import torch
import wandb
from data import get_dataset_gaze,get_small_dataset
...
def fsdp_main(rank, world_size, args):
    ...
    #dataset loading 
    train_dataset_info = get_small_dataset(tokenizer, clip_processor, 'train', rank, world_size,sample_limit=32*10)
    ...
    checkpoint_path = os.path.join("insert path/open_flamingo/open_flamingo/models/small_training_base/001_last_checkpoint.pth")
    if os.path.isfile(checkpoint_path):
        dist.barrier()
        resume_checkpoint=1
        start_epoch ,checkpoint = load_checkpoint_gaze("insert path/open_flamingo/open_flamingo/models/small_training_base/001_last_checkpoint.pth")
        if rank==0:
            model.load_state_dict(checkpoint['model_state_dict'],strict=False)
    ...
    #training 
        ...
        trainloss= train_one_epoch_v3(args, model, epoch, train_dataset, tokenizer,optimizer, scheduler, rank, None)  # Assuming wandb or logging handled within
        train_loss.append(trainloss)

        print("saving started")
        if(trainloss < best_loss):
            best_loss=trainloss
            save_checkpoint_gaze(model, optimizer, scheduler, epoch, "insert path/open_flamingo/open_flamingo/models/small_training_base/001_modified_last_checkpoint.pth")
        if rank==0:
            save_loss_to_file(trainloss,f"{args.save_dir}/small_base_train_loss.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir",type=str,default="insert path/open_flamingo/open_flamingo/models/small_training_base")
    ...
    parser.add_argument("--step_size", type=int, default=100, help="Step size for LR scheduler")
    parser.add_argument("--gamma", type=float, default=0.0, help="Gamma for LR scheduler")
