...
import torch
import wandb
from data import get_dataset_new
...
def fsdp_main(rank, world_size, args):
    ...
    #dataset loading 
    train_dataset_info = get_dataset_new(tokenizer, clip_processor, 'train', rank, world_size)
    ...
    checkpoint_path = os.path.join("insert path/open_flamingo/open_flamingo/models/caption/base_model/base_model_checkpoint_10.pth")
    if os.path.isfile(checkpoint_path):
        dist.barrier()
        resume_checkpoint=1
        start_epoch ,checkpoint = load_checkpoint_base("insert path/open_flamingo/open_flamingo/models/caption/base_model/base_model_checkpoint_10.pth")
        if rank==0:
    ...
    #training 
        ...
        if( trainloss < best_loss):
            best_loss=trainloss
            save_checkpoint_gaze(model, optimizer, scheduler, epoch, "insert path/open_flamingo/open_flamingo/models/caption/base_model/base_model_checkpoint_10.pth")
        if rank==0:
            save_loss_to_file(trainloss,f"{args.save_dir}/base_train_loss_new.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir",type=str,default="insert path/open_flamingo/open_flamingo/models/caption/base_model")
