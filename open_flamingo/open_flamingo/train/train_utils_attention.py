...
def train_gaze_attention(args, model, epoch, custom_loader, tokenizer, optimizer, lr_scheduler, device_id, wandb):
    ...
    for num_steps, batch in enumerate(custom_loader):
        ...
        with torch.cuda.amp.autocast(enabled=args.precision != "fp32"):
            images,overlays,gaze,input_ids, attention_mask, labels = prepare_batch_gaze_attention(batch, tokenizer, device_id,image_token_id,endofchunk_token_id)
            outputs,attn_weights,cosine_sim = model(images,overlays,input_ids, attention_mask, labels)
            ...
            loss = (outputs.loss+ reg_value*kl_loss + 0) / args.gradient_accumulation_steps
        ...
        if device_id==0 and (num_steps % 100 == 0):
            print(
                f"Step {num_steps+1} of epoch {epoch}/{args.epochs} complete. Loss is: {loss.item():.3f}")
            print("KL loss is ",kl_loss)
            # print("loss from gaze generation is ",cosine_sim)    
            print("loss from outputs is ",outputs.loss)                
...
def evaluate_model_blip(test_loader, model, processor, device_id, generation_config):
    # model.eval()
    ...
    with torch.inference_mode():
        with torch.cuda.amp.autocast():  
            ...
            for num_steps, batch in enumerate(test_loader):
                # Prepare the batch
                #print("checkpoint1")
                inputs,labels = prepare_batch_gaze_attention(
                # images,input_ids, attention_mask, labels = prepare_batch_gaze_attention(

                    batch, processor, device_id,
                    image_token_id=processor("<image>", add_special_tokens=False)["input_ids"][-1],
                    endofchunk_token_id=processor("<|endofchunk|>", add_special_tokens=False)["input_ids"][-1]
                )
                # ground_truth = decode_token_ids_new(processor,input_ids)
                # print(ground_truth)
                # Generate predictions using the new model's chat function
                prompt = (
                    "<image><image><image><image><image> These are a sequence of images showing 5 seconds.Please predict what happens in the next 2 seconds.The annotations should correspond to what happens in the next 2 seconds and should describe the action taking place,objects interacting, and the trajectory of the camera wearer and the output should only consist of 2 seconds"
                    # "objects interacting, and the trajectory of the camera wearer " "
                )


                # Original shape: [32, 5, 1, 3, 448, 448]
                # Remove the unexpected dimension and flatten batch size and sequence length
                images = images.squeeze(2)  # Remove the extra dimension: [32, 5, 3, 448, 448]
                images = images.view(-1, *images.shape[2:])  # Flatten to [32 * 5, 3, 448, 448]
                ...
                ground_truth = processor.batch_decode(labels, skip_special_tokens=True)[0].strip()


                #print(predicted_text)

                    # Compute semantic score for each prediction
                minibatch_scores = []
                for pred_text, gt_text in zip(predicted_text, ground_truth):
...
def evaluate_model(test_loader, model, tokenizer, device_id, generation_config):
    model.eval()
    ...
    with torch.inference_mode():
        with torch.cuda.amp.autocast():  
            ...
            for num_steps, batch in enumerate(test_loader):
                # Prepare the batch
                #print("checkpoint1")
                images, overlays, gaze, input_ids, attention_mask, labels = prepare_batch_gaze_attention(
                # images,input_ids, attention_mask, labels = prepare_batch_gaze_attention(

                    batch, tokenizer, device_id,
                    image_token_id=tokenizer("<image>", add_special_tokens=False)["input_ids"][-1],
                ...
                predicted_text = model.chat(
                    tokenizer, images, prompt, generation_config
                )
                #print(predicted_text)
...
def evaluate_model_rouge(test_loader, model, tokenizer, device_id, generation_config):
    model.eval()
    ...
    with torch.inference_mode():
        with torch.cuda.amp.autocast():
            ...
            for num_steps, batch in enumerate(test_loader):
                ...
                # Define the prompt
                prompt = (
                    "<image><image><image><image><image> These are a sequence of images showing 5 seconds."
                    " Please predict what happens in the next 2 seconds. The annotations should correspond"
                    " to what happens in the next 2 seconds and should describe the action taking place,"
                    " objects interacting, and the trajectory of the camera wearer."
                )

                # Adjust the shape of images to match model requirements
                images = images.squeeze(2)  # Remove the extra dimension: [32, 5, 3, 448, 448]
                images = images.view(-1, *images.shape[2:])  # Flatten to [32 * 5, 3, 448, 448]

                # Generate predictions using the model's chat function
                predicted_text = model.chat(tokenizer, images, prompt, generation_config)
                ...
                # Compute the average ROUGE-L scores for the mini-batch
