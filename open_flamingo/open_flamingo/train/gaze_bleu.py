...
import torch
import wandb
from data import get_dataset_gaze_eval,get_dataset_gaze,get_small_dataset,get_dataset_new ,get_dataset_gaze_attention
...
def fsdp_main(rank, world_size, args):
    ...
    test_dataset_info = get_dataset_gaze_attention(tokenizer, clip_processor, 'test', rank, world_size,base_path='/home/pani3/dataset_agg')
    test_loader = test_dataset_info.dataloader

    checkpoint_path = os.path.join("insert path/open_flamingo/open_flamingo/models/gaze_attention_agg/gaze_model_sreg.pth")
    if os.path.isfile(checkpoint_path):
        dist.barrier()
        checkpoint = load_checkpoint_eval("insert path/open_flamingo/open_flamingo/models/gaze_attention_agg/gaze_model_sreg.pth")
    ...
    total_scores = score_meteor_gaze(model,test_loader,tokenizer,rank)
    if rank==0:
        save_loss_to_file(total_scores,"insert path/open_flamingo/open_flamingo/models/gaze_attention_agg/gaze_model_sreg_meteor.txt")
        print("over")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=str, default="1925", help="Distributed training port")
