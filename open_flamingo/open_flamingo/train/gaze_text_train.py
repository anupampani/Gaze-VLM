...
def fsdp_main(rank, world_size, args):
    ...
    checkpoint_path = os.path.join("insert path/open_flamingo/open_flamingo/models/gaze_text/gaze_text_model10.pth")
    if os.path.isfile(checkpoint_path):
        dist.barrier()
        resume_checkpoint=1
        start_epoch ,checkpoint = load_checkpoint_base("insert path/open_flamingo/open_flamingo/models/gaze_text/gaze_text_model10.pth")
        if rank==0:
    ...
    #training 
        ...
        if( trainloss < best_loss):
            best_loss=trainloss
            save_checkpoint_gaze(model, optimizer, scheduler, epoch,"insert path/open_flamingo/open_flamingo/models/gaze_text/gaze_text_model10.pth")
        if rank==0:
            save_loss_to_file(trainloss,f"{args.save_dir}/gaze_text_loss.txt")
