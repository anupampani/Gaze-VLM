...
import torch
import wandb
from data import get_dataset_gaze_eval,get_dataset_gaze,get_small_dataset,get_dataset_new ,get_dataset_gaze_attention,get_dataset_text
...
def fsdp_main(rank, world_size, args):
    ...
    checkpoint_path = os.path.join("insert path/open_flamingo/open_flamingo/models/base_wgazeoverlaid/base_model_checkpoint_10.pth")
    if os.path.isfile(checkpoint_path):
        dist.barrier()
        checkpoint = load_checkpoint_eval("insert path/open_flamingo/open_flamingo/models/base_wgazeoverlaid/base_model_checkpoint_10.pth")
    ...
    total_scores = score_rouge_base(model,test_loader,tokenizer,rank)
    if rank==0:

        save_loss_to_file(total_scores,"insert path/open_flamingo/open_flamingo/models/base_wgazeoverlaid/base_model_rogue.txt")
        print("over")
