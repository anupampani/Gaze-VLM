...
def main():
    ...
    # Initialize model - to-do :change to my model 
#     # Initialize data loaders
    ...
    # Initialize data loaders - to-do: introduce my dataset here
    custom_dataset = get_dataset1(split= 'train', tokenizer=tokenizer, clip_processor=clip_processor)   
    ...
    total_training_steps = (
        (num_samples) // (custom_dataset.dataloader.batch_size* args.world_size)
    ) * args.num_epochs
    if args.rank == 0:
    ...
    # Start training!
    ddp_model.train()
    #to-do: introduce my epoch function
    for epoch in range(args.num_epochs):
        train_one_epoch_new(args,
            model=model,
            epoch=epoch,
            custom_loader=custom_dataset.dataloader,  # Pass the custom loader
            optimizer=optimizer,
            tokenizer=tokenizer,
            ...
            # Pass other necessary arguments (device, scheduler, etc.)
        )        
