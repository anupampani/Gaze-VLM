...
def load_checkpoint_base(load_path):
    checkpoint=None
...
def fsdp_main(rank, world_size, args):
    ...
    #dataset loading 
    train_dataset_info = get_dataset_text(tokenizer, clip_processor, 'train', rank, world_size)
    ...
    checkpoint_path = os.path.join("insert path/open_flamingo/open_flamingo/models/gaze_text/gaze_text_model210.pth")
    if os.path.isfile(checkpoint_path):
        dist.barrier()
        resume_checkpoint=1
        start_epoch ,checkpoint = load_checkpoint_base("insert path/open_flamingo/open_flamingo/models/gaze_text/gaze_text_model210.pth")
        if rank==0:
    ...
    #training 
        ...
        trainloss= train_one_epoch_v3(args, model, epoch, train_dataset, tokenizer,optimizer, scheduler, rank, wandb)  # Assuming wandb or logging handled within
        train_loss.append(trainloss)
        # validloss= validate(args, model, valid_dataset, tokenizer, rank)
        # valid_loss.append(validloss)
        print("saving started")
        # if(validloss < best_loss):
        if( trainloss < best_loss):
            best_loss=trainloss
            save_checkpoint_gaze(model, optimizer, scheduler, epoch,"insert path/open_flamingo/open_flamingo/models/gaze_text/gaze_text_model210.pth")
        if rank==0:
            save_loss_to_file(trainloss,f"{args.save_dir}/gaze_text_loss2.txt")
            # save_loss_to_file(validloss,f"{args.save_dir}/base_valid_loss_new.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port", type=str, default="1728", help="Distributed training port")
    ...
    parser.add_argument("--save_dir",type=str,default="insert path/open_flamingo/open_flamingo/models/gaze_text")
