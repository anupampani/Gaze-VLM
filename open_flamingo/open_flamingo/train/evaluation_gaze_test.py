...
import torch
import wandb
from data import get_dataset_gaze_eval,get_dataset_gaze,get_small_dataset,get_small_dataset_gaze,get_dataset_gaze_attention
...
def fsdp_main(rank, world_size, args):
    ...
    # test_dataset_info = get_dataset_gaze(tokenizer, clip_processor, 'test', rank, world_size,32)
    test_dataset_info = get_dataset_gaze_attention(tokenizer, clip_processor, 'test', rank, world_size,base_path='insert path/datasets/dataset_gaze_sharegpt')
    test_loader = test_dataset_info.dataloader



    checkpoint_path = os.path.join("insert path/open_flamingo/open_flamingo/models/sharegpt/agg/gaze_model_10.pth")
    if os.path.isfile(checkpoint_path):
        dist.barrier()
        checkpoint = load_checkpoint_eval("insert path/open_flamingo/open_flamingo/models/sharegpt/agg/gaze_model_10.pth")
    ...
    text = text_generate_debug(model,test_loader,tokenizer,rank)
    print("over")
    cleanup_distributed()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir",type=str,default="insert path/open_flamingo/open_flamingo/models/sharegpt/agg/")
